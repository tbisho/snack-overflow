...
module.exports = {
  up: async (queryInterface, Sequelize) => {
    ...
    const bulkUsers = await queryInterface.bulkInsert('user', [
      {
    ...
    await queryInterface.bulkDelete('snack', null, {truncate: true, cascade: true, restartIdentity: true});

    const bulkSnacks = await queryInterface.bulkInsert('snack', [
      {
        name: 'Paleta Payaso',
    ...
    console.log('snack: ', bulkSnacks);

    const bulkUserSnacks = await queryInterface.bulkInsert('user_snack', [
      {
    ...
    const bulkComments = await queryInterface.bulkInsert('comment', [
      {
        userId: bulkUsers[0].id,
        snackId: bulkSnacks[0].id
        text: 'Danish jelly-o bonbon. Gummies pastry soufflé tootsie roll caramels caramels sweet roll. Halvah fruitcake jelly-o sweet roll caramels tootsie roll. Apple pie lollipop muffin jelly beans biscuit chocolate.' ,
        createdAt: new Date(),
        updatedAt: new Date(),
      }, {
        userId: bulkUsers[0].id,
